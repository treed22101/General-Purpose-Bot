...
#setting our levels and exp
    ...
    async def on_message(self, message):
        ...
        if not author_id in self.users:
            ...
            self.users[author_id]['Level 1'] = 1
...
#level command (lets us know our level and exp)
    ...
    async def level(self,ctx,user:discord.User=None):
        ...
        embed = discord.Embed(
            title = f"{user.name}'s Level & Exp",
            color=discord.Color.dark_red
        )
        embed.add_field(name='Level:', value=self.users[str(user.id)]['Level'])
        embed.add_field(name='Exp:', value=self.users[str(user.id)]['Exp'])
